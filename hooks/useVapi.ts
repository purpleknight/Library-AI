...
export function useVapi(book: IBook) {
   ...
   const start = useCallback(async () => {
      ...
      try {
         ...
         // The actual limit is enforced by useLatestRef(limits.maxDurationPerSession * SECONDS_PER_MINUTE)
         ...
         console.error('Failed to start call:', err);
            if (sessionIdRef.current) {
               endVoiceSession(sessionIdRef.current, 0).catch((endErr) => console.error(`Failed to
                  rollback voice session after start failure:`, endErr),);
               sessionIdRef.current = null;
            }
         setStatus('idle');
